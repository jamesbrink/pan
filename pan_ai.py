...
class PanAI:
    ...
    def __init__(self):
        ...
        print(
            f"Loading model {self.model_name} with context length {self.context_length} and quantization {self.quantization_level}"
        )

        # Configure tokenizer with appropriate context length
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_name, model_max_length=self.context_length
        )

        # Set up quantization configuration using the utility function
        quantization_config, bits = pan_utils.create_quantization_config(
    ...
    def generate_response(self, prompt, max_length=100):
        ...
        outputs = self.model.generate(
            ...
            + len(inputs.input_ids[0]),  # Account for prompt length
